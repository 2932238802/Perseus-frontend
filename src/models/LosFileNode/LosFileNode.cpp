...
LosFileNode::LosFileNode(const QString &filePath, LosFileNode *parent)
    : LOS_parent(nullptr), LOS_filePath(filePath) {
  if (parent)
...
*/
void LosFileNode::setParentNode(LosFileNode *node) { LOS_parent = node; }
...
获取元素
*/
int LosFileNode::getChildCount() const { return LOS_childrens.size(); }
...
获取
*/
LosFileNode *LosFileNode::getParent() const { return LOS_parent; }
