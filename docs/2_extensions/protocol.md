...
Perseus 插件系统采用**远程市场 + 本地加载 + 命令注入**的架构：

```
+------------------+      +-------------------+      +------------------+
|   远程插件市场    | ---> |  LosNet (网络层)   | ---> |  LosPlugUi 列表   |
|  (插件元数据/包)  | <--- |  请求/下载/README  | <--- | LosPluginDetailUi|
+------------------+      +-------------------+      +------------------+
                                                            │
                                                            ▼
                                                  +------------------+
...
- **网络交互**由 `LosCore::LosNet`（单例）承担：
  - `requestPlugin()`：拉取插件市场列表 → `_cmd_net_pluginReply(QList<PluginInfo>)`
  - `fetchPluginReadme(url, cb)`：拉取插件 README
  - `downloadPlugin(url, savePath)`：下载插件包 → `_cmd_net_downloadFinished(path)`
- **UI 展示**：`LosPlugUi`（列表）+ `LosPluginDetailUi`（详情，`_cmd_openPluginDetail`）
- **命令注入**：插件命令经 `LosCommandUi::regis(...)` 注册到命令面板
